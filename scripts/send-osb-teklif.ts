...
import { getOutreachMailTemplate, getOutreachPdfPath } from "../src/lib/outreach-templates";
import { resolvePublicAttachment, sendTitanMail } from "../src/lib/titan-mail";

const SEND_DELAY_MS = 800;
const PDF_PATTERN = /^(0[1-9]|[1-4][0-9]|50)_/;
...
async function main() {
  ...
  console.log(`OSB teklif gönderim: ${targets.length} OSB (henüz mail atılmamış)`);
  console.log("");
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    const to = company.email?.trim() || template?.to?.trim() || "";
