...
async function main() {
  ...
  console.log(`Bekleyen firma: ${pending.length}`);
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      const text = appendSignatureText(content.bodyText);
      const html = appendSignatureHtml(content.bodyHtml, undefined, { embedded: true });
      ...
      console.log(`✓ ${company.name} → ${to}`);
      sent++;
    } catch (err) {
      console.log(
        `✗ ${company.name} — ${err instanceof Error ? err.message : "gönderim hatası"}`
      );
      failed++;
    }

    if (i < pending.length - 1) await sleep(SEND_DELAY_MS);
  }
